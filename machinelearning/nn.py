...
class Graph(object):
    ...
    def get_inputs(self, node):
        ...
        if len(node.get_parents()) != 0:
            for p in node.get_parents():
                inputs.append(p.forward(self.get_inputs(p)))
            return inputs
        elif type(node) is DataNode:
            return node.data
        return []
    ...
    def get_output(self, node):
        ...
        """
        return self.outputs[self.nodes.index(node)]
    ...
    def get_gradient(self, node):
        ...
        """
        return self.gradients[self.nodes.index(node)]
    ...
    def add(self, node):
        ...
        self.gradients.append(np.zeros_like(output))
    ...
    def step(self, step_size):
        ...
        for v in self.variables:
            v.data = v.data - step_size * self.gradients[self.nodes.index(v)]
...
class ReLU(FunctionNode):
    ...
    @staticmethod
    def backward(inputs, gradient):
        x = np.maximum(inputs[0], np.zeros_like(inputs[0]))
...
class SquareLoss(FunctionNode):
    ...
    def backward(inputs, gradient):
        x, y = inputs
        return [((x - y) * gradient) / x.size, -((x - y) * gradient) / x.size]
