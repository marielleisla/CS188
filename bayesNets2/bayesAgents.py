...
class VPIAgent(BayesAgent):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        for a in factor.getAllPossibleAssignmentDicts():
            ...
            if a[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and a[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                leftExpectedValue += v * WON_GAME_REWARD
                rightExpectedValue += v * GHOST_COLLISION_REWARD
            elif a[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and a[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
                leftExpectedValue += v * GHOST_COLLISION_REWARD
                rightExpectedValue += v * WON_GAME_REWARD
            else:
                continue
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for prob, explorationEvidence in self.getExplorationProbsAndOutcomes(evidence):
            explorationEvidence.update(evidence)
            l, r = self.computeEnterValues(explorationEvidence, enterEliminationOrder)
            if l > r:
                expectedValue += prob * l
            else:
                expectedValue += prob * r
